...
Backstage also aligns with the Ports and Adapters pattern through its swappable infrastructure: the Keyv cache abstraction hides which concrete backend is in use (Redis, Memcache, Valkey, or in-memory); the `SignInResolver` interface decouples identity logic from the choice of provider such as Okta or GitHub OAuth; and TechDocs storage adapters work interchangeably across AWS S3, GCS, Azure, or local filesystem storage. This allows infrastructure decisions to be deferred or changed without touching plugin logic.
